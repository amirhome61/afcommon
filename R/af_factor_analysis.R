...
af_interpret_cronbach <- function(alpha_result) {
  ...
  interpretation <- sprintf(
    "%s (%.2f)",
    labels[findInterval(raw_alpha, breaks)],
    raw_alpha
  )
...
af_cfa <- function(
  ...
  fit_predict <- lavaan::lavPredict(
    ...
    append.data = TRUE,
    transform = TRUE
  ) # predictions incorporate the model-implied mean structure
  ...
  latent_predict <- target_min +
    (latent_scores - latent_min) *
...
af_check_order <- function(
  ...
  for (g in names(fit_predict)) {
    ...
    for (var in var_list) {
      if (!all(df[group_indices, var] == fit_predict[[g]][, var])) {
        all_errors <- c(
          all_errors,
          paste("Mismatch in", var, "for group", g, "\n")
        )
        all_correct <- FALSE
      }
    }
    ...
    if (
      !all(latent_scores[group_indices] == fit_predict[[g]][, latent_var_name])
    ) {
      all_errors <- c(
        all_errors,
        paste("Mismatch in latent scores for group", g, "\n")
      )
      all_correct <- FALSE
    }
  }
  ...
  if (all_correct) {
    all_errors <- c(
      all_errors,
      paste("All checks passed - ordering is correct!\n")
    )
  }
...
af_create_sem_plot <- function(
  ...
  if (edge_labels) {
    # Do not inclue the edgeLabels parameter
    p <- semPlot::semPaths(
      fit, # model to plot
      exoVar = FALSE,
      exoCov = FALSE,
...
af_gt_cfa_results_tbl <- function(cfa_tbl) {
  gt_table <- gt(cfa_tbl) %>%
    gt::tab_source_note(
      source_note = "CFI (Comparative Fit Index) values above 0.90 indicate good fit."
    ) %>%
    gt::tab_source_note(
      source_note = "TLI (Tucker-Lewis Index) values above 0.90 indicate good fit."
    ) %>%
    gt::tab_source_note(
      source_note = "RMSEA (Root Mean Square Error of Approximation) values below 0.08 indicate reasonable fit, below 0.05 indicate good fit."
    ) %>%
    gt::tab_source_note(
      source_note = "SRMR (Standardized Root Mean Residual) values below 0.08 indicate good fit."
    )
...
af_clean_cfa_model <- function(df, model) {
  ...
  clean_line <- function(line) {
    ...
    if (length(parts) != 2) {
      return(line)
    } # Return unchanged if not a measurement model line
...
af_measurement_invariance <- function(
  ...
  configural_fit <- af_cfa(
    df = df,
    var_list = var_list,
    latent_var_name = latent_var_name,
    group = group,
    model = model,
    cfa_type = "configural"
  )
  ...
  latent_comparison_plot <- ggplot() +
    geom_smooth(
      aes(
        x = 1:length(configural_fit$latent_predict),
    ...
    geom_smooth(
      aes(
        x = 1:length(metric_fit$latent_predict),
    ...
    geom_smooth(
      aes(
        x = 1:length(scalar_fit$latent_predict),
    ...
    labs(
      title = "Comparison of Latent Predictions",
      x = "Index",
      y = "Latent Score",
      color = "Model Type"
    ) +
  ...
  metric_config_diff <- metric_fit$latent_predict -
    configural_fit$latent_predict
  scalar_metric_diff <- scalar_fit$latent_predict - metric_fit$latent_predict

  metric_config_cor <- cor(
    metric_fit$latent_predict,
    configural_fit$latent_predict
  )
  ...
  metric_config_plot <- ggplot() +
    ...
    geom_hline(
      yintercept = mean(metric_config_diff),
      linetype = "dashed",
      color = "red"
    ) +
    geom_hline(
      yintercept = min(metric_config_diff),
      linetype = "dotted",
      color = "blue"
    ) +
    geom_hline(
      yintercept = max(metric_config_diff),
      linetype = "dotted",
      color = "blue"
    ) +
    labs(
      title = "Metric - Configural Differences",
  ...
  scalar_metric_plot <- ggplot() +
    ...
    geom_hline(
      yintercept = mean(scalar_metric_diff),
      linetype = "dashed",
      color = "red"
    ) +
    geom_hline(
      yintercept = min(scalar_metric_diff),
      linetype = "dotted",
      color = "blue"
    ) +
    geom_hline(
      yintercept = max(scalar_metric_diff),
      linetype = "dotted",
      color = "blue"
    ) +
    labs(
      title = "Scalar - Metric Differences",
...
af_interpret_measurement_invariance <- function(
  ...
  rownames(chi_tests) <- c(
    "fit.config",
    "fit.metric",
    "fit.scalar",
    "fit.strict"
  )
  rownames(fit_indices) <- c(
    "fit.config",
    "fit.metric",
    "fit.scalar",
    "fit.strict"
  ...
  scalar_evaluation <- list(
    ...
    borderline_cfi = abs(
      ...
      borderline_cfi,
    acceptable_fit = fit_indices["fit.scalar", "cfi"] > 0.95 &&
      fit_indices["fit.scalar", "rmsea"] < borderline_rmsea
  )
  ...
  interpretation <- paste0(
    ...
    if (all(unlist(metric_evaluation))) {
      if (
        all(unlist(scalar_evaluation[c("chi_p", "absolute_fit")])) &&
          (scalar_evaluation$borderline_cfi || scalar_evaluation$acceptable_fit)
      ) {
