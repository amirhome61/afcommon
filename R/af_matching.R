af_interpret_matching <- function(
  ...
  if (!is.null(max_smd_squares)) {
    balance_text <- paste0(
      balance_text,
      sprintf(" and all standardized mean differences for squares and two-way interactions between covariates were below %.2f", 
              max_smd_squares)
    )
  }
  ...
  units_text <- if (!units_discarded) {
    sprintf("%s uses all treated and all control units, so no units were discarded by the matching.", 
            match_method)
  } else {
...
# The function `af_match_compare` performs comparative analysis of treatment effects across multiple survey waves, with and without matching. For each wave, it estimates treatment effects on multiple dependent variables using linear regression, controlling for specified covariates. It then performs nearest-neighbor matching on these covariates and re-estimates the treatment effects. The function outputs both a dataframe containing detailed regression results (estimates, p-values, and confidence intervals) and a visualization comparing matched and unmatched treatment effects across waves and outcomes. The plot displays treatment estimates with confidence intervals, using asterisks to denote statistical significance levels (* p<0.05, ** p<0.01, *** p<0.001).

af_match_compare <- function(df, dep_list, ctrl_vars, treat_var, treat_level, group_var, group_list) {
  ...
  for(group in group_list) {
    ...
    for(match_type in c("Unmatched", "Matched")) {
      ...
      if(match_type == "Matched") {
        match_formula <- as.formula(paste(treat_var, "~", paste(ctrl_vars, collapse = " + ")))
      ...
      for(dep_var in dep_list) {
        # Create formula
        formula_str <- paste(dep_var, "~", treat_var, "+", paste(ctrl_vars, collapse = " + "))
  ...
  plot <- ggplot(results_df, aes(x = dependent, y = estimate, 
