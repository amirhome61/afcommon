...
af_plot_interaction <- function(model, dep_label, x_label, y_label, title = "") {
  sjPlot::plot_model(model, type = "int", title = title,
                     axis.title = c(x_label, y_label), legend.title = dep_label) + 
    theme(legend.position = "bottom")
}
...
af_modelsummary <- function(models, 
                            tag = "reg-out", 
                            coef_map = NULL, 
                            coef_exp = FALSE, 
                            exp_ctrl = NULL,
  ...
  if (!is.null(exp_ctrl)) {
    ...
    notes_text <- paste("Estimates of models", toString(exp_ctrl_idxs), "are exponentiated.")
  }
...
af_create_regression_notes <- function(df, models, display_names = NULL, 
                                       is_robust = FALSE, is_bootstrap = FALSE, 
                                       bootstrap_iterations = 1000, is_exp = FALSE, 
                                       show_significance = FALSE, show_n = FALSE,
  ...
  if (show_significance) {
    note_parts$significance <- paste0(
      "Note: * p < ", format(max(significance_levels), nsmall = 3), "; ",
      "** p < ", format(median(significance_levels), nsmall = 3), "; ",
      "*** p < ", format(min(significance_levels), nsmall = 3)
    )
  }
  ...
  get_reference_levels <- function(model) {
    ...
    factor_vars <- all_vars[all_vars %in% names(df) & sapply(df[all_vars], is.factor)]
    
    ref_levels <- character()
    for (var in factor_vars) {
      ref_level <- levels(df[[var]])[1]  # assuming first level is reference
      display_var <- ifelse(!is.null(display_names[var]), display_names[var], var)
      ref_levels <- c(ref_levels, 
                      sprintf("The reference category for %s is '%s'", 
                              display_var, ref_level))
    }
    return(ref_levels)
  }
  ...
  if (show_n) {
    if (inherits(models, "list")) {
      ...
      if (n_models > 1) {
        if (n_models == 2) {
          note_parts$n <- sprintf("N = %d and %d for models 1–2, respectively",
                                  sample_sizes[1], sample_sizes[2])
        } else {
          note_parts$n <- sprintf("N = %s, and %d for models 1–%d, respectively",
                                  paste(sample_sizes[-n_models], collapse = ", "),
                                  sample_sizes[n_models], n_models)
        }
      }
    } else {
      # Single model case
  ...
  if (any(is_robust)) {
    if (inherits(models, "list")) {
      ...
      if (length(robust_models) > 1) {
        ...
        note_parts$robust <- sprintf("Models %s have robust standard errors in parenthesis", model_nums)
      } else {
        model_nums <- robust_models
        note_parts$robust <- sprintf("Model %s has robust standard errors in parenthesis", model_nums)
      }
    } else {
  ...
  if (any(is_bootstrap)) {
    if (inherits(models, "list")) {
      ...
      if (length(bootstrap_models) > 1) {
        ...
        note_parts$bootstrap <- sprintf("Models %s have bootstrapped standard errors in parentheses (%d iterations)", model_nums, bootstrap_iterations)
      } else {
        model_nums <- robust_models
        note_parts$bootstrap <- sprintf("Model %s has bootstrapped standard errors in parenthesis (%d iterations)", model_nums, bootstrap_iterations)
      }
    } else {
      note_parts$bootstrap <- sprintf("Bootstrapped standard errors in parenthesis (%d iterations).", bootstrap_iterations) 
    }
  }
  ...
  if (any(is_exp)) {
    if (inherits(models, "list")) {
      ...
      if (length(exp_models) > 1) {
        ...
        note_parts$exp <- sprintf("The coefficients of %s are exponentiated", model_nums)
      } else {
...
af_hetero_test <- function(model) {
  ...
  if (pval < 0.05) {
    result_line <- paste0("We use Breusch-Pagan test for homoscedasticity (equal variance) assumption. The result of P-value ", round(pval, 3), " < 0.05 indicates a concern for heteroscedasticity.\n")
  } else {
    result_line <- paste0("We use Breusch-Pagan test for homoscedasticity (equal variance) assumption. The result of P-value ", round(pval, 3), " >= 0.05, indicates homoscedasticity (no concern for heteroscedasticity).\n")
  }
