...
af_scatter_map <- function(
  ...
  if (
    !is.factor(df[[identity_var_name]]) &&
      !is.character(df[[identity_var_name]])
  ) {
  ...
  if (is.null(color_var_name)) {
    p <- ggplot(df, aes(x = !!sym(x_var_name), y = !!sym(y_var_name))) +
      geom_point(size = 3, color = "black")
  } else {
    # Ensure color variable is a factor or character
    if (
      !is.factor(df[[color_var_name]]) && !is.character(df[[color_var_name]])
    ) {
  ...
  p <- p +
    geom_text(
      aes(label = !!sym(identity_var_name)),
      hjust = -0.3,
      vjust = 0.5,
      size = 3
    )
