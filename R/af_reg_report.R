...
{
  if (reg_type == "OLS") af_rr_ols(model, df, y_name, x_list, digits = digits)
  else if (reg_type == "NB") af_rr_nb(model, df, y_name, x_list)
  else if (reg_type == "Poisson") af_rr_poisson(model, df, y_name, x_list)
  else if (reg_type == "polr") af_rr_polr(model, df, y_name, x_list, digits = digits)
  else if (reg_type == "logistic") af_rr_logistic(model, df, y_name, x_list, digits = digits)
  else paste("regression type",reg_type, "is not supported")
}
...
af_rr_ols <- function(model, df, y_name, x_list, digits = 2)
{
  ...
  replacement <- c("<reg_type>" = reg_type, 
                   "<dep_name>" = dep_name,
  ...
  for (i in 1:numx)
  {
    if (reg_sig[i]) {
      ...
      if (cov_cat=="") { # Non-Categorical variable
      ...
      rr_shiny_text <- paste(rr_shiny_text,
                             af_rr_par(tmpl_cov, replacement), 
                             sep = '<br/>')
    } else {
      cov_name <- names(x_list[i])
      cov_cat <- as.character(x_list[i])
      if (cov_cat!="") cov_name <- paste0(cov_name,"[", cov_cat,"]")
      cov_val <- exp(reg_coef[i])
      cov_pval <- reg_pv[i]
      
      replacement <- c("<cov_name>" = cov_name,
...
af_rr_nb <- function(model, df, y_name, x_list)
{
  ...
  for (i in 1:numx)
  {
    if (reg_sig[i]) {
      ...
      cov_aval <- abs(cov_val)
      if (cov_type=="factor") 
        cov_cat<-af_last(levels(df[,x_list[i]])) else cov_cat <- ""
      cov_dir <- af_select_val(cov_val>0, "increases", "decreases")
...
af_rr_poisson <- function(model, df, y_name, x_list)
{
  ...
  for (i in 1:numx)
  {
    if (reg_sig[i]) {
      ...
      cov_aval <- abs(cov_val)
      if (cov_type=="factor") 
        cov_cat<-af_last(levels(df[,cov_name])) else cov_cat <- ""
      cov_dir <- af_select_val(cov_val>0, "increases", "decreases")
