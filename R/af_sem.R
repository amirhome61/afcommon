af_sem_interpretation <- function(model) {
  ...
  predictor_interpretations <- lapply(1:nrow(regressions), function(i) {
    ...
    paste0(path$rhs, " (", round(path$std.all, 3), ") ", effect_direction, " ", path$lhs, " by ", round(path$est, 3))
  })
  
  # Assemble summary interpretation
  interpretation <- paste0(
    "\n\nIn summary, ", 
    strongest_predictor$rhs, " (", round(strongest_predictor$std.all, 3), 
    ...
    ifelse(cfi >= 0.90 & tli >= 0.90 & rmsea <= 0.08 & srmr <= 0.08, "acceptable", "poor"), 
